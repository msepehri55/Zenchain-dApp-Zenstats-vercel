...
function isSwap(tx, rcpt) {
  ...
  if (fn.includes('swap')) return true;
...
function isAddLiquidity(tx, rcpt) {
  ...
  const sig = String(tx.input || '').slice(0,10).toLowerCase();
  const fn = String(tx.functionName || '').toLowerCase();
  if (hasAnyTopic(rcpt, new Set([TOPIC_UNIV2_MINT.toLowerCase()]))) return true;
...
function isRemoveLiquidity(tx, rcpt) {
  ...
  const sig = String(tx.input || '').slice(0,10).toLowerCase();
  const fn = String(tx.functionName || '').toLowerCase();
  if (hasAnyTopic(rcpt, new Set([TOPIC_UNIV2_BURN.toLowerCase()]))) return true;
  if (fn.includes('removeliquidity')) return true;
...
function isApprove(tx, rcpt) {
  ...
  const sig = String(tx.input || '').slice(0,10).toLowerCase();
  const fn = String(tx.functionName || '').toLowerCase();
  if (fn === 'approve' || fn.includes('allowance')) return true;
  if (APPROVE_SELECTORS.has(sig)) return true;
  // Fallback to Approval event
  for (const lg of rcpt?.logs || []) {
    const t0 = (lg.topics?.[0] || '').toLowerCase();
    if (t0 === TOPIC_ERC20_APPROVAL.toLowerCase()) return true;
  }
  return false;
}
...
export async function buildActivity({ address, start, end }) {
  ...
  // fail > cc/cco/gm (overrides) > domain > cc (heuristic) > ci > stake > dex swap/add/remove > gm > approve > native_send > nft > other
  const nativeRows = [];
  for (const t of externals) {
    const rcpt = rcptMap.get(String(t.hash).toLowerCase());
...
export async function buildStats({ address, start, end }) {
  ...
  return {
    ...
    gmCount:      countCat('gm'),
    ccCount:      ccBoth,
    // Optional extra counts (not shown on the home KPIs unless you wire them)
