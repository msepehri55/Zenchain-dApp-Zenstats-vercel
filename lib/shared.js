...
// Decode possible 721A/2309 addresses (topics preferred, fallback data)
function parse721AAddresses(lg) {
  try {
    const t = (lg.topics || []).map(x => String(x || '').toLowerCase());
    // If indexed addresses exist in topics, they are usually topics[2] and topics[3]
    if (t.length >= 4) {
      const fromT = addrFromTopic(t[2]);
      const toT   = addrFromTopic(t[3]);
      if (fromT && toT) return { from: fromT, to: toT };
    }
    // Fallback: try last two 32-byte words from data as addresses
    const d = String(lg.data || '').toLowerCase();
    if (d && d.startsWith('0x')) {
      const WORD = 64;
      if (d.length >= 2 + WORD * 2) {
        const w3 = d.slice(d.length - WORD * 2, d.length - WORD);
        const w4 = d.slice(d.length - WORD);
        const from = ('0x' + w3.slice(24));
        const to   = ('0x' + w4.slice(24));
        if (/^0x[0-9a-f]{40}$/.test(from) && /^0x[0-9a-f]{40}$/.test(to)) return { from, to };
      }
    }
  } catch {}
  return null;
...
function mintedContractsForUser(rcpt, user) {
  ...
  for (const lg of rcpt?.logs || []) {
    ...
    if (t0 === TOPIC_ERC721_TRANSFER.toLowerCase()) {
      ...
      if (pair && pair.from === ZERO.toLowerCase() && pair.to === user) {
        set.add(String(lg.address || '').toLowerCase());
      }
...
function isAddLiquidityQuick(tx) {
  const sig = inputSig(tx);
  const fn = String(tx.functionName || '').toLowerCase();
  // IMPORTANT: do NOT match on "mint" here (causes NFT mint mislabels)
...
// Quick candidates for mint/domain (to decide which receipts to fetch)
...
async function hydrateMissingExternalsFromInternals({ internalsFeed, externals, address, limit = 180 }) {
...
async function hydrateMissingExternalsFromTokenFeeds({ token20, token721, token1155, externals, limit = 240 }) {
  const extSet = new Set((externals || []).map(t => String(t.hash || '').toLowerCase()));
  const tsMap = new Map();
...
// Core: build activity (fast path first, receipts only for confirming native sends + mint/domain)
export async function buildActivity({ address, start, end }) {
  const addr = address.toLowerCase();

  // Fetch all feeds in parallel (faster cold start)
  ...
  // Hydrate missing externals (kept but capped to reduce RPC pressure)
  externals = await hydrateMissingExternalsFromInternals({ internalsFeed, externals, address: addr, limit: 180 });
  externals = await hydrateMissingExternalsFromTokenFeeds({ token20: erc20, token721: erc721, token1155: erc1155, externals, limit: 240 });
  ...
  const mintDomainCandidates = new Set();    // hashes likely to be mint/domain, confirm via receipts
  ...
  for (const t of externals || []) {
    ...
    if (isFailedByTxlist(t)) {
      ...
      // Mark potential mint/domain tx for targeted receipt confirmation
      if (category === 'other' && ((isNFTMintQuick(t) || isDomainQuick(t)))) {
  ...
  // Confirm potential native sends and mint/domain candidates (single receipts pass)
