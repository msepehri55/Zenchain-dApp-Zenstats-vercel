...
const codeCache    = new Map(); // addressLower -> bytecode or '0x' (kept for future)
...
// Explorer paging (dynamic to old windows)
...
// NFT mint selectors (721/721A/1155 common)
...
async function getReceiptsCached(hashes, max = 2000, concurrency = 8, retries = 2) {
  const out = [];
  const miss = [];
...
// Minimal native send (fast; avoids EOA bytecode checks)
function isNativeSendSimple(tx) {
  const from = (tx.from || '').toLowerCase();
  const to = (tx.to || '').toLowerCase();
  const val = BigInt(tx.value || '0');
  if (!to) return false;
  if (to.startsWith('0x0000000000000000000000000000000000000')) return false;
  if (val <= 0n) return false;
  if (!/^0x0*$/i.test(tx.input || '0x')) return false;
  return true;
}
...
// Hydration: internals; token feeds (mints only) for speed
...
async function hydrateMissingExternalsFromTokenFeedsMintsOnly({ token721, token1155, externals, address, limit = 300 }) {
  ...
  for (const n of token721 || []) {
    ...
    if (from === ZERO.toLowerCase() && to === addr) {
      const h = String(n.hash || n.transactionHash || '').toLowerCase();
      if (h && !extSet.has(h)) want.add(h);
    }
  }
  ...
  const missing = [...want].slice(0, limit);
  if (!missing.length) return externals;
...
export async function buildActivity({ address, start, end }) {
  ...
  // Inputs then minimal hydration (mints-only from token feeds)
  ...
  externals = await hydrateMissingExternalsFromTokenFeedsMintsOnly({ token721: erc721, token1155: erc1155, externals, address: addr, limit: 300 });

  // Receipts (keep to detect fails and some topics)
  ...
  // Internals by tx (for deploy/creation)
  ...
  // Domain / NFT mints from feeds (fast)
