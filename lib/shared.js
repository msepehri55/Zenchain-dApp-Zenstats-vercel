...
// Hydrate missing externals (old tx that appear only in internal feed)
async function hydrateMissingExternalsFromInternals({ internalsFeed, externals, address, limit = 400 }) {
  ...
  const hyd = fetched.filter(Boolean);
  return [...externals, ...hyd];
}
...
export async function buildActivity({ address, start, end }) {
  ...
  // Hydrate missing old externals using internals
...
// Stats derived from activity (OUTGOING externals onlyy)
