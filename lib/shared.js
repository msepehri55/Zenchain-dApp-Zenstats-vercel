...
// Fast native send candidate (we confirm via receipts later)
...
// Core: build activity (fast path first, receipts only for what we must confirm)
export async function buildActivity({ address, start, end }) {
  ...
  const domainTxFromFeed = new Set(
    (Array.isArray(erc721) ? erc721 : [])
      ...
      .filter(n => {
        const sym = String(n.tokenSymbol || '').toLowerCase();
        const name = String(n.tokenName || '').toLowerCase();
        return sym.includes('.zen') || sym.includes('.ztc') || name.includes('.zen') || name.includes('.ztc');
      })
  ...
  const nativeRows = [];
  const nativeConfirmSet = new Set(); // hashes that might be native and need receipt confirm (logs length == 0)
  ...
  // We will augment deploy suspects later if needed for creation check (uses internals only; no RPC)
  ...
  for (const t of externals || []) {
    ...
    if (isFailedByTxlist(t)) {
      ...
      if (domainTxFromFeed.has(txhashLower)) {
        ...
        // provisional, will be confirmed by receipt (logs == 0)
      } else {
        // creation (heuristic using internals only)
  ...
  // Confirm potential native sends by fetching receipts only for those hashes
  ...
  // Tag token rows as nft_mint if ZERO -> you (so mint appears even if native row was "other")
