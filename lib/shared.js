...
// Client (serverless-safe)
const client = createPublicClient({ transport: http(RPC) });

// Caches (best effort; serverless may reuse warm instances)
const receiptCache = new Map(); // hash -> receipt
const codeCache = new Map();    // addr -> '0x...' or '0x' (EOA)
const metaCache = new Map();    // nft addr -> {symbol,name}
...
export async function fetchPagedAccount(action, address, { startTs, endTs }, opts = {}) {
  ...
  if (startTs === 0) maxPages = Math.max(maxPages, 250); // All time: deeper paging
...
// CONTRACT OVERRIDES (add more here as needed)
...
// Classification
const emptyInput = (d) => /^0x0*$/i.test(d || '0x'); // SINGLE DEFINITION (fixed)
const isPrecompile = (to) => (to || '').toLowerCase().startsWith('0x0000000000000000000000000000000000000');
const looksLikeDomainText = (s) => {
  const x = String(s || '').toLowerCase();
  return x.includes('.ztc') || x.includes('.zen') || x.includes('zns') || x.includes('domain') || x.includes('name') || x.includes('.');
};
...
export async function buildActivity({ address, start, end }) {
  ...
  // fail > (contract overrides) cc/cco/gm > domain > cc (heuristic) > ci > stake > gm (signature) > native_send > nft > other
  const nativeRows = [];
  for (const t of externals) {
    const rcpt = rcptMap.get(String(t.hash).toLowerCase());
