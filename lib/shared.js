...
export async function buildActivity({ address, start, end }) {
  ...
  const [internalsFeed, erc20, erc721, erc1155] = await Promise.all([
    ...
    // ERC-1155 feed (some explorers support this Etherscan-compatible action)
  ...
  // Domain mints by feed (ERC-721 only)
  ...
  for (const t of externals || []) {
    ...
    if (isFailed(rcpt)) {
      ...
      if (isDomain) {
        ...
        // Fallback: mark ZERO -> user NFT transfers as NFT mint when not a domain
        category = 'nft_mint';
