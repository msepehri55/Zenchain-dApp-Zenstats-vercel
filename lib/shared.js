...
function isNativeSend(tx, rcpt, user, eoaMap) {
  ...
  if (!rcpt || (rcpt.logs || []).length === 0) return true;
...
export async function buildActivity({ address, start, end }) {
  ...
  // Hydrate from internals and token feeds for older windows
  ...
  // Generic NFT mints by feed (ERC-721)
  ...
  for (const t of externals || []) {
    ...
    if (isFailed(rcpt)) {
      ...
      } else if (rcpt && mintedContractsForUser(rcpt, addr).size > 0) {
        category = 'nft_mint';
      }
    }
