...
const ADD_LIQ_SELECTORS = new Set([
  ...
  // V3 mainly via events (Mint/IncreaseLiquidity)
]);
...
const REM_LIQ_SELECTORS = new Set([
  ...
  // V3 handled via DecreaseLiquidity event
]);
...
// NOTE: We intentionally do NOT use isCI anymore for speed and to avoid mislabels.
// export async function isCI(...) { ... } // kept out
...
function isSwap(tx, rcpt) {
  if (rcpt && isFailed(rcpt)) return false; // if we know it failed, ignore
  const sig = inputSig(tx);
  const fn = String(tx.functionName || '').toLowerCase();
...
export async function buildActivity({ address, start, end }) {
  ...
  // fail > cc/cco/gm (overrides) > domain > cc (heuristic) > stake > dex swap/add/remove > gm(sig) > approve > native_send > nft > other
  const nativeRows = [];
  for (const t of externals) {
    const rcpt = rcptMap.get(String(t.hash).toLowerCase());
    ...
    if (isFailed(rcpt)) {
      ...
      // REMOVED: CI check (expensive and noisy)
...
export async function buildStats({ address, start, end }) {
  ...
  // Only outgoing external tx count towards KPIs (your original behavior)
