...
// Speed toggles
const ENABLE_HYDRATE = false;          // set true only if you must backfill missing externals (slower)
const RECEIPT_CONCURRENCY = 12;        // receipts for native_send + mint/domain confirmation
...
export async function fetchPagedAccount(action, address, { startTs, endTs }, opts = {}) {
  let pageSize = opts.pageSize ?? 100;         // keep 100 (zentrace behaves like etherscan)
...
// -------------------- ERC721 meta (unused on the hot path) --------------------
const ERC721_ABI = [
...
// Domain/NFT quick hints
function looksLikeDomainText(s) {
  if (!s) return false;
...
function isDomainQuick(tx) {
  ...
  return DOMAIN_SELECTORS.has(sig);
}
function isNFTMintQuick(tx) {
  const sig = inputSig(tx);
  const fn = String(tx.functionName || '').toLowerCase();
  if (fn.includes('mint') || fn.includes('claim')) return true;
  return NFT_MINT_SELECTORS.has(sig);
}

// Robust checks with receipt
...
function isAddLiquidityQuick(tx) {
  const sig = inputSig(tx);
  const fn = String(tx.functionName || '').toLowerCase();
  // IMPORTANT: do NOT match on "mint" (that caused NFT mints to be mislabeled)
...
async function hydrateMissingExternalsFromInternals({ internalsFeed, externals, address, limit = 120 }) {
...
async function hydrateMissingExternalsFromTokenFeeds({ token20, token721, token1155, externals, limit = 160 }) {
  const extSet = new Set((externals || []).map(t => String(t.hash || '').toLowerCase()));
  const tsMap = new Map();
...
// -------------------- Receipts (selective) --------------------
async function getReceiptsSelective(hashes, concurrency = RECEIPT_CONCURRENCY, retries = RECEIPT_RETRIES) {
  const out = new Map();
  const limit = pLimit(concurrency);
  await Promise.all([...(new Set(hashes))].map(h => limit(async () => {
...
export async function buildActivity({ address, start, end }) {
  ...
  // Optional hydration (disabled by default for speed)
  if (ENABLE_HYDRATE) {
    externals = await hydrateMissingExternalsFromInternals({ internalsFeed, externals, address: addr, limit: 120 });
    externals = await hydrateMissingExternalsFromTokenFeeds({ token20: erc20, token721: erc721, token1155: erc1155, externals, limit: 160 });
  }

  // Pre-map internals by hash to detect CC (contract creation)
  ...
  // First pass
  const nativeRows = [];
  const nativeConfirmSet = new Set();     // confirm native_send (logs==0)
  const mintDomainCandidates = new Set(); // confirm NFT/domain (receipt-based)
  ...
  for (const t of externals || []) {
    ...
    if (isFailedByTxlist(t)) {
      ...
      // Possible contract creation by this user (via internals)
      ...
      // Stronger candidate rule for mint/domain:
      // Any OUTGOING external contract call we still see as "other", with non-empty input OR unknown method,
      // is a candidate (covers explorers missing methodId/functionName).
      const isOutgoing = fromLower === addr;
      const toIsContractCall = !!t.to && !isPrecompile(t.to);
      const hasNonEmptyInput = !emptyInput(t.input);
      if (category === 'other' && isOutgoing && toIsContractCall && (hasNonEmptyInput || isNFTMintQuick(t) || isDomainQuick(t))) {
        mintDomainCandidates.add(txhashLower);
      }
    }
  ...
  // Single receipts pass for native_send + mint/domain
  ...
  if (toConfirm.length) {
    ...
    // mint/domain confirmation (receipt-based, no reliance on functionName)
