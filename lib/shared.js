...
// RPC helpers with retries (speed-tuned)
async function getReceiptsCached(hashes, max = 1800, concurrency = 10, retries = 2) {
  const out = [];
  const miss = [];
...
export async function buildEOAMap(address, externals) {
  // Kept for compatibility, but we no longer call it for speed.
  const map = new Map();
  return map;
}
...
// Fast native send (no EOA bytecode fetch)
function isNativeSendFast(tx, rcpt, user) {
  ...
  // EOA transfers have no logs; payable contract with empty input usually emits logs
  if (rcpt && (rcpt.logs || []).length > 0) return false;
  return true;
}
...
export async function buildActivity({ address, start, end }) {
  ...
  // Receipts (fast, concurrent) for success/logs/topics
  ...
  for (const t of externals || []) {
    ...
    if (rcpt && isFailed(rcpt)) {
      ...
      if (isDomain) {
        ...
        category = 'approve';
      } else if (isNativeSendFast(t, rcpt, addr)) {
