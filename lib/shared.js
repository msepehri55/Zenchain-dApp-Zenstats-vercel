...
const GM_CONTRACTS = toLowerSet([
  ...
  '0x1290b4f2a419a316467b580a088453a233e9adcc', // zkCodes GM
]);
...
// NEW: NFT mint selectors (721/721A/1155 common)
...
export async function buildActivity({ address, start, end }) {
  ...
  // IMPORTANT: tag token rows with 'nft_mint' if feed shows ZERO -> you
  ...
  const token721 = (Array.isArray(erc721) ? erc721 : []).map(n => {
    ...
    return {
      ...
      valueNorm: 0,
      // Tag token rows as nft_mint if the feed says ZERO -> you
  ...
  const prio = (r) => {
    ...
    if (r.kind === 'token')    return (r.category === 'nft_mint') ? 85 : 20; // elevate token nft_mint so it appears if native row is missing
