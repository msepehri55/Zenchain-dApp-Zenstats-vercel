...
export async function fetchPagedAccount(action, address, { startTs, endTs }, opts = {}) {
  ...
  while (true) {
    ...
    // Stop if we reached/before start boundary or this is the last page
...
async function getReceiptsCached(hashes, max = 1200, concurrency = 8, retries = 2) {
  const out = [];
  const miss = [];
...
// Hydrate missing externals (1) from internals, (2) from token feeds
...
async function hydrateMissingExternalsFromTokenFeeds({ token20, token721, externals, limit = 600 }) {
  const extSet = new Set((externals || []).map(t => String(t.hash || '').toLowerCase()));
  const tsMap = new Map(); // hash -> timestamp (from token feed)
...
export async function buildActivity({ address, start, end }) {
  ...
  // Ensure inputs for what we have
  externals = await fillMissingInputs(externals);
  // Hydrate missing old externals from internals and token feeds
  externals = await hydrateMissingExternalsFromInternals({ internalsFeed, externals, address: addr, limit: 400 });
  externals = await hydrateMissingExternalsFromTokenFeeds({ token20: erc20, token721: erc721, externals, limit: 600 });
  ...
  // fail > cc/cco/gm (overrides) > domain > cc (heuristic) > stake > swap/add/remove > gm(sig) > approve > native_send > nft > other
  const nativeRows = [];
  for (const t of externals || []) {
    const rcpt = rcptMap.get(String(t.hash || '').toLowerCase());
