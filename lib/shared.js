...
// Normalize arrays to lowercase Set
const toLowerSet = (arr) => new Set(arr.map((s) => String(s || '').toLowerCase()));

// Safe selector (supports tx.input, or explorer methodId, or empty)
const inputSig = (tx) => {
  const raw = tx?.input ?? tx?.methodId ?? '0x';
  return String(raw).slice(0, 10).toLowerCase();
};
...
// V3 liquidity management (NonfungiblePositionManager) events
...
const ADD_LIQ_SELECTORS = new Set([
  ...
  // V3 often uses mint() on position manager; we rely mainly on events for V3
]);
...
const REM_LIQ_SELECTORS = new Set([
  ...
  // V3 decreaseLiquidity handled via events
]);
...
export async function fillMissingInputs(externals) {
  ...
  for (const t of externals) {
    if (!t.input) {
      ...
      // Some explorers also give methodId; keep our inputSig fallback as extra safety
    }
  }
  return externals;
}
...
function isSwap(tx, rcpt) {
  ...
  const sig = inputSig(tx);
  const fn = String(tx.functionName || '').toLowerCase();
  if (hasAnyTopic(rcpt, new Set([
    TOPIC_UNIV2_SWAP.toLowerCase(),
    TOPIC_UNIV3_SWAP.toLowerCase()
  ]))) return true;
...
function isAddLiquidity(tx, rcpt) {
  ...
  const fn = String(tx.functionName || '').toLowerCase();
  if (hasAnyTopic(rcpt, new Set([
    TOPIC_UNIV2_MINT.toLowerCase(),
    TOPIC_UNIV3_MINT.toLowerCase(),
    TOPIC_UNIV3_INCREASE_LIQ.toLowerCase()
  ]))) return true;
...
function isRemoveLiquidity(tx, rcpt) {
  ...
  const fn = String(tx.functionName || '').toLowerCase();
  if (hasAnyTopic(rcpt, new Set([
    TOPIC_UNIV2_BURN.toLowerCase(),
    TOPIC_UNIV3_DECREASE_LIQ.toLowerCase()
  ]))) return true;
...
function isApprove(tx, rcpt) {
  ...
  // Fallback to Approval events (ERC20, ERC721, ERC1155)
  for (const lg of rcpt?.logs || []) {
    const t0 = (lg.topics?.[0] || '').toLowerCase();
    if (
      t0 === TOPIC_ERC20_APPROVAL.toLowerCase() ||
      t0 === TOPIC_ERC721_APPROVAL.toLowerCase() ||
      t0 === TOPIC_ERC1155_APPROVAL_ALL.toLowerCase()
    ) return true;
  }
  return false;
}
...
export async function buildActivity({ address, start, end }) {
  ...
  // fail > cc/cco/gm (overrides) > domain > cc (heuristic) > ci > stake > dex swap/add/remove > gm(sig) > approve > native_send > nft > other
  const nativeRows = [];
  for (const t of externals) {
    const rcpt = rcptMap.get(String(t.hash).toLowerCase());
...
// Stats derived from activity to avoid double counting
export async function buildStats({ address, start, end }) {
  const activity = await buildActivity({ address, start, end });
  const ext = activity.filter(r => r.kind === 'native' && r.category !== 'fail');

  const countCat = (name) => ext.filter(r => r.category === name).length;
  const ccBoth = ext.filter(r => r.category === 'cc' || r.category === 'cco').length;
