...
// Optional allow-list for domain registrars (empty by default; fill when known)
...
async function promoteMintCategoriesWithReceipts({ externals, nativeRows, address }) {
  ...
  for (const t of externals || []) {
    ...
    if (r.category && r.category !== 'other' && r.category !== 'approve') continue;
    ...
    // Heuristic: prioritize likely mint calls; still allow some "other" to be scanned
  ...
  const rcpts = await getReceiptsSelective(targets, 10, 2);
...
export async function buildActivity({ address, start, end }) {
  ...
  for (const t of externals || []) {
    ...
    if (isFailedByTxlist(t)) {
      ...
      category = 'domain_mint'; // conservatively treat registrar calls as domain-related
    } else {
  ...
  if (nativeConfirmSet.size > 0) {
    ...
    for (const h of nativeConfirmSet) {
      ...
      if (!isFailed(rcp) && (!rcp.logs || rcp.logs.length === 0)) {
        // Only set to native_send if not already identified as mint/domain/etc.
  ...
  const prio = (r) => {
    ...
    if (r.kind === 'token') {
      ...
      // but native mint will win now after promotion.
